...
private $host = "localhost";
  ...
  private $password = "TrB6uXzz";
  private $database = "ecocompare";
...
$results = mysql_query($query);
        ...
        $query="SELECT DISTINCT A.id,A.ean,DATE_FORMAT(B.date,'%Y-%m-%d') as date_request,A.description,A.marque,A.produitvert,A.noresult FROM  iphone_eans_desc A,iphone_query B where A.ean=B.ean and A.ean and A.description=''  and length(A.ean)=13 and A.noresult=$noresult   order by B.date desc,marque asc  limit $limit";
        $results = mysql_query($query);
...
/*DEBUT AJOUT BADGE*/
	...
	function insertAvisBadge($idpdt, $avis, $numQuestion){
		$query = "INSERT INTO badge_avis SET product_id='$idpdt', avis='$avis', num_question='$numQuestion' ";
		mysql_query($query);
		echo mysql_error();
	}
